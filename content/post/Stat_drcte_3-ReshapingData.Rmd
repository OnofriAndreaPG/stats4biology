...
This is a follow-up post. If you are interested in other posts of this series, please go to: [https://www.statforbiology.com/tags/drcte/](https://www.statforbiology.com/tags/drcte/)
...
#kable(dataset, align = rep("c", 17), caption = "Table 1. Example of a common field-book for germination assays: the data is organised with one Petri dish per row and the number of germinated seeds at each inspection time along the columns.")
```
...
datasetG <- melt_te(lotusOr, count_cols = 3:17, treat = Stage, 
                   monitimes = 1:15, subjects = rep(25,12))
head(datasetG, 16)
```
...
```{r}
datasetU <- melt_te(lotusOr, count_cols = 3:17, treat = 1, 
                   monitimes = 1:15, subjects = rep(25,12), grouped = F)
head(datasetU, 16)
```
