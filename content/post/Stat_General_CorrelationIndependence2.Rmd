...
# Still not able to fit with mmes!
...
# Fit as bivariate (it does not work?)
mod.bimix2 <- mmes(cbind(Yield, TKW) ~ 1,
...
# Fit as univariate (better, but cannot fit unstructured residuals)
dataset$Plot <- 1:81
mdataset <- dataset |>
...
mdataset <- as.data.frame(mdataset)
head(mdataset)
mdataset <- arrange(mdataset, Trait)
mdataset$un <- factor(1:162)
mod.bimix2 <- mmes(Y ~ Trait - 1,
                   random = ~vsm(usm(Trait), ism(Genotype)) +
                     vsm(usm(Trait), ism(Block)),
                   rcov   = ~ dsm(usm(Trait), ism(units)),
                   data = mdataset,
                   verbose = FALSE, dateWarning = FALSE)
bimix.obj2 <- summary(mod.bimix2)
coefs2 <- bimix.obj2$varcomp
coefs2
```
...
# Correlation between genotype means (no longer working!)
# I used the function `vpredict()` and specified the necessary combinations of variance-covariance parameters. It is necessary to remember that estimates, in ‘sommer’, are named as V1, V2, … Vn, according to their ordering in model output.
...
mod.bimix2 <- mmer(cbind(Yield, TKW) ~ 1,
                   ...
                   data = dataset,
                  verbose = FALSE, dateWarning = FALSE)
summary(mod.bimix2)$varcomp
# Likelihood ratio test
LRT <- anova(mod.bimix, mod.bimix2)
AIC(mod.bimix, mod.bimix2)
...
mod.bimix3 <- mmer(cbind(Yield, TKW) ~ 1,
                   ...
                   data = dataset,
                  verbose = FALSE, dateWarning = FALSE)
summary(mod.bimix3)$varcomp
# Likelihood ratio test
logLikFull <- summary(mod.bimix)$logo$logLik
logLikReduced <- summary(mod.bimix3)$logo$logLik
LRT <- -2*(logLikFull-logLikReduced)
LRT
...
# Likelihood ratio test
logLikFull <- summary(mod.bimix)$logo$logLik
logLikReduced <- summary(mod.bimix4)$logo$logLik
...
5. Wickham H, Vaughan D, Girlich M (2024). _tidyr: Tidy Messy Data_.doi:10.32614/CRAN.package.tidyr <https://doi.org/10.32614/CRAN.package.tidyr>, R package version 1.3.1, <https://CRAN.R-project.org/package=tidyr>.
