...
Let's assume that we have a randomised blocks design, with 27 genotypes and 3 replicates. For each plot, we recorded two traits, i.e. yield and the weight of thousand kernels (TKW). In the end, we have 81 plots and just as many couples of measures in all. We will use the dataset 'WheatQuality', that is available on  the 'statforbiology' package.
...
We observe a positive correlation, and $r$ seems to be significantly different from 0. Therefore, we would be encouraged to conclude that plots with a high value on yield tend to have a high value on TKW, as well. Unfortunately, such an inference is not supported by the data.
...
```{r}
means <- aggregate(dataset[,c("Yield", "TKW")],
                   list(dataset$Genotype), 
                   mean)
...
This post was originally written on 2019-04-30 and updated on 2026-04-15 
