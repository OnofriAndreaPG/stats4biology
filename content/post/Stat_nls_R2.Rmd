...
Bearing these two limitations in mind, there is no reason why we should not use such a goodness-of-fit measure with nonlinear regression. In this line, the `R2.nls()` function in the 'statforbiology' package can be used to retrieve the R^2^ and Pseudo-R^2^ values from a nonlinear model fitted with the `nls()` and `drm()` functions.
