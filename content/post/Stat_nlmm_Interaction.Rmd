...
The dataset is available on 'statforbiology', which is the accompanying package for this blog and it is available on CRAN. It is composed by the following variables:
...
For this post, we will use several other packages, which are indicated at the beginning of the following code box; next, the dataset is loaded and the 'Block', 'N' and 'GEN' variables are transformed into factors. 
...
dataset <- getAgroData("growthNGEN")
...
To start the analyses, it may be useful to take a look at the observed growth data, as displayed on the graph below.


```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
# xyplot(Yield ~ DAS|N:GEN, data = dataset) old coding, now ggplot is preferred
ggplot(dataset) +
  geom_point(aes(y = Yield, x = DAS)) +
  facet_grid(N~GEN) +
  theme_bw()
```
...
This model may be useful for other circumstances (no blocks and no repeated measures), but it is wrong in our example. Indeed, observations are clustered within blocks and plots; by neglecting this, we violate the assumption of independence of model residuals. Swift plots of residuals against fitted values and QQ-plot show that there are no problems with heteroscedasticity and normality, although a few outliers might deserve careful inspection (which we will neglect now). Note that these two graphs are obtained using the `plotRes()` function in 'statforbiology'.

```{r echo=FALSE, fig.align='center', out.width='90%'}
par(mfrow = c(1,2))
plotRes(modNaive1, which = 1)
plotRes(modNaive1, which = 2)
```
...
1. A deterministic function. In this case, we use the `NLS.L3()` function in the 'statforbiology' package, which provides a logistic growth model with the same parameterisation as the `L.3()` function in the 'drc' package.
...
# Create data.frame
df <- expand.grid(DAS=X, GEN=LETTERS[1:3], N=c("High", "Low"))
df$Yield <- c(p1,p2,p3,p4,p5,p6)

# Plot everything
ggplot(dataset) +
  geom_point(aes(y = Yield, x = DAS)) +
  geom_line(data = df, mapping = aes(y = Yield, x = DAS),
            col = "red") +
  facet_grid(N~GEN) +
  theme_bw()
```
...
Happy coding with R! ... and ... don't forget to check out my new book!
...
---
Originally written on 13 September 2019; revised on February 2025
