...
I have already considered the #1 in a previous post ([follow this link here](https://www.statforbiology.com/2021/stat_nls_paircomp/)); basically, there are functions in the 'drc' (`compParm()`) and 'statforbiology' (`compCoefs(),` `pairComp()` and `gnlht()`) packages that permit pairwise comparisons across model parameters.
...
In the box below. we install the 'statforbiology' package from gitHub (if necessary), load it and load the 'metamitron' dataset.
