...
Although there are certainly more exhaustive solutions, for the sake of simplicity, I have coded the `GGE()` function, that is available in the `statforbiology` package; it works equally well with raw MET data, containing all replicated values, or with the 'genotype by environment' average values. The `statforbiology` package is the companion package for this website and must have been installed as detailed in this [page here](https://www.statforbiology.com/rpackages/). In the box below, I am using the `GGE()` function on raw data.
...
This is great! Now we have the ability of drawing a biplot, i.e.  we can plot both genotypic scores and environmental scores in a dispersion graph (biplot: two plots into one), as we see below. We can use the swift `biplot()` method for AMMI and GGE objects, as available in the `statforbiology` package.
...
Thank you for reading, and... happy coding! And ... don't forget to check out my new book!
