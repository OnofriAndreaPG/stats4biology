...
In R, the 'Pseudo-R^2^' can be calculated by using the `R2.nls()` function in the 'statforbiology' package, for nonlinear models fitted with both the `nls()` and `drm()` functions (this latter function is in the 'drc' package).
...
Hope this was useful; for those who are interested in the use of the Pseudo-$R^2$ in nonlinear regression, I hav already published one post at this link: [https://www.statforbiology.com/2021/stat_nls_r2/](https://www.statforbiology.com/2021/stat_nls_r2/).
