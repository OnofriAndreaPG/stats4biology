...
# Parametrizzazione (prova, ma errore)
dataset$Herbicide
dataset$Adjuvant
dataset$DoseF
Xmat <- model.matrix(model2)
coef(model2)
dataset$Herbicide2 <- factor(ifelse(dataset$Herbicide == "HandWeeded" | dataset$Herbicide == "Unweeded", NA, dataset$Herbicide))
dataset$DoseF2 <- factor(ifelse(dataset$DoseF == "NONE", NA, dataset$DoseF))
dataset$Adjuvant2 <- factor(ifelse(dataset$Adjuvant == "None", NA, dataset$Adjuvant))
model3 <- lm(Yield ~ CvT/(Herbicide2*DoseF2*Adjuvant2),
             data = dataset)
anova(model2)
```
