...
\tightlist
\item
  poniamo di avere un parametro \(\beta\) da stimare ed otteniamo lo stimatore campionario \(b\) = 12);
\item
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{anova}\NormalTok{(}\FunctionTok{lm}\NormalTok{(Count }\SpecialCharTok{\textasciitilde{}}\NormalTok{ Insecticide, }\AttributeTok{data =}\NormalTok{ dataset))}
\DocumentationTok{\#\# Analysis of Variance Table}
\DocumentationTok{\#\# }
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{mod}\FloatTok{.1} \OtherTok{\textless{}{-}} \FunctionTok{lm}\NormalTok{(}\FunctionTok{sqrt}\NormalTok{(Count) }\SpecialCharTok{\textasciitilde{}}\NormalTok{ Insecticide, }\AttributeTok{data =}\NormalTok{ dataset)}
\NormalTok{mod}\FloatTok{.2} \OtherTok{\textless{}{-}} \FunctionTok{lm}\NormalTok{((Count}\SpecialCharTok{\^{}}\FloatTok{0.5} \SpecialCharTok{{-}} \DecValTok{1}\NormalTok{)}\SpecialCharTok{/}\FloatTok{0.5} \SpecialCharTok{\textasciitilde{}}\NormalTok{ Insecticide, }\AttributeTok{data =}\NormalTok{ dataset)}
...
\item
  Siegel, S e Castellan N.J. (1992) Statistica non-parametrica. McGraw-Hill, Milano.
\end{enumerate}
...
\DecValTok{2} \SpecialCharTok{*} \FunctionTok{pt}\NormalTok{(}\FunctionTok{abs}\NormalTok{(Tval), }\DecValTok{12}\NormalTok{, }\AttributeTok{lower.tail =}\NormalTok{ F)}
...
\hypertarget{esempio-2-confronti-multipli-con-dati-trasformati}{%
\section{Esempio 2: confronti multipli con dati trasformati}\label{esempio-2-confronti-multipli-con-dati-trasformati}}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{prima }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}\FloatTok{113.5}\NormalTok{, }\FloatTok{116.5}\NormalTok{, }\FloatTok{118.1}\NormalTok{, }\FloatTok{111.3}\NormalTok{, }\FloatTok{116.8}\NormalTok{, }\FloatTok{117.0}\NormalTok{, }\FloatTok{114.8}\NormalTok{, }\FloatTok{114.9}\NormalTok{, }
           \FloatTok{114.8}\NormalTok{, }\FloatTok{114.2}\NormalTok{, }\FloatTok{115.0}\NormalTok{, }\FloatTok{114.0}\NormalTok{, }\FloatTok{114.4}\NormalTok{, }\FloatTok{116.1}\NormalTok{, }\FloatTok{117.9}\NormalTok{, }\FloatTok{115.7}\NormalTok{)}
\NormalTok{dopo }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}\FloatTok{110.9}\NormalTok{, }\FloatTok{110.1}\NormalTok{, }\FloatTok{110.3}\NormalTok{, }\FloatTok{116.8}\NormalTok{, }\FloatTok{112.2}\NormalTok{, }\FloatTok{111.0}\NormalTok{, }\FloatTok{111.1}\NormalTok{, }\FloatTok{112.9}\NormalTok{, }
          \FloatTok{110.6}\NormalTok{, }\FloatTok{109.1}\NormalTok{, }\FloatTok{113.1}\NormalTok{, }\FloatTok{109.9}\NormalTok{, }\FloatTok{111.9}\NormalTok{, }\FloatTok{110.1}\NormalTok{, }\FloatTok{114.2}\NormalTok{, }\FloatTok{111.0}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\hypertarget{fitting-di-modelli-nella-sperimentazione-agraria}{%
\section{Fitting di modelli nella sperimentazione agraria}\label{fitting-di-modelli-nella-sperimentazione-agraria}}
...
\item
  Descrivere i dati, calcolando almeno un indicatore di tendenza centrale (media) e un indicatore di variabilità (deviazione standard) come indicato nel Capitolo 3.
\item
...
\item
  Testare la significanza di tutti gli effetti inclusi nel modello, attraverso l'ANOVA.
\item
  Se necessario, confrontare le medie con un test di confronto multiplo
\item
  Presentare i risultati e commentarli.
\end{enumerate}

I dataset degli esempi che seguono sono riportati in un file Excel, che potete scaricare dal seguente link: \url{https://www.casaonofri.it/_datasets/BookExercises.xlsx}. Ogni dataset è su un diverso foglio di lavoro all'interno dello stesso file; il nome del foglio di lavoro è riportato in fondo ad ogni esercizio. Utilizzare la funzione `readxl()' function, per la lettura del dataset.
...
\caption{\label{tab:unnamed-chunk-221}Sommario dei principali modelli per descrivere i principali esperimenti nelle scienze agrarie}
\centering
\begin{tabular}[t]{l|l|l|l}
\hline
Modello & Disegno.sperimentale & Funzione.R & Specifiche\\
\hline
ANOVA ad una via & CRD & lm() & Y \textasciitilde{} F1\\
\hline
ANOVA ad una via & CRBD & lm() & Y \textasciitilde{} F1 + BL\\
\hline
ANOVA a due vie & CRD & lm() & Y \textasciitilde{} F1 * F2\\
\hline
ANOVA a due vie & CRBD & lm() & Y \textasciitilde{} F1 * F2 + BL\\
\hline
ANOVA a due vie & Split-plot CRD & lmer() & Y \textasciitilde{} F1 * F2 + (1|MAIN)\\
\hline
ANOVA a due vie & Split-plot CRBD & lmer() & Y \textasciitilde{} F1 * F2 + BL + (1|MAIN)\\
\hline
ANOVA a due vie & Strip-plot CRD & lmer() & Y \textasciitilde{} F1 * F2 + (1|ROW) + (1|COL)\\
\hline
ANOVA a due vie & Strip-plot CRBD & lmer() & Y \textasciitilde{} F1 * F2 + BL + (1|ROW + (1|COL)\\
\hline
ANOVA ad una via & ANOVA ad una via (CRD), due ambienti & lm()/lmer() & Y \textasciitilde{} F1 * ENV\\
\hline
ANOVA ad una via & ANOVA ad una via (CRBD), due ambienti & lm()/lmer() & Y \textasciitilde{} F1 * ENV + BL|ENV\\
\hline
Regressione Lineare semplice & CRD & lm() & Y \textasciitilde{} X1\\
\hline
...
{[}Sheet: 7.3{]} \emph{SUGGERIMENTO: in questo esercizio è necessaria una trasformazione stabilizzante che non è automaticamente riconosciuta da `emmeans()'. Pertanto, per la retrotrasformazione, sarà necessario utilizzare la procedura descritta alla fine del paragrafo 9.8.}
...
Eseguite l'ANOVA. Quali sono le assunzioni necessarie per l'ANOVA? Sono rispettate? Vi sono outliers? Calcolate SEM e SED in modo attendibile. \emph{SUGGERIMENTO: in questo esercizio è necessaria una trasformazione stabilizzante che non è automaticamente riconosciuta da `emmeans()'. Pertanto, per la retrotrasformazione, sarà necessario utilizzare la procedura descritta alla fine del paragrafo 9.8.}
{[}Sheet: 7.4{]}
...
{[}Sheet: 10.2{]} \emph{SUGGERIMENTO: in questo dataset vi è un dato mancante, contrassegnato con NA (Not Available). Al contrario della funzione `read.csv()', la funzione `read\_xlsx()' non è in grado di riconoscere autometicamente questo dato mancante e, pertanto, sarà necessario utilizzare largomento \texttt{na\ =\ "NA"}, all'interno del comando di lettura file.}
...
{[}Sheet: 10.3{]} \emph{SUGGERIMENTO: l'esperimento è a quadrato latino e, di conseguenza, vi sono tre fonti di variazione additive: il fertilizzante, la riga e la colonna (riguardare il display dei disegni a quadrato latino al capitolo 2, se necessario).}
