...
and it is available as the `DRC.powerCurve()` and `NLS.powerCurve()` functions in the 'statforbiology' package.
...
The logarithmic function can be fitted by using 'lm()', by using a log-transformed predictor. If necessary, it can also be fit by using 'nls()' and 'drm()', with the self-starting functions `NLS.logCurve()` and `DRC.logCurve()`, which are available within the 'statforbiology' package.
