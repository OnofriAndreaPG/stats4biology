...
tags:
  - 'Split_plot'
aliases:
  - /2020/stat_nlmm_designconstraints/index.html
---
...
dataset <- getAgroData("yieldDensityB")
...
```{r}
rm(list=ls())
dataset <- getAgroData("YieldLossB")
dataset$block <- factor(dataset$block)
head(dataset)
...
where $YL$ is the yield loss, $D$ is weed density, $i$ is the slope at the origin of axes and $a$ is the maximum asymptotic yield loss. This function, together with self-starters, is available in the 'NLS.YL()' function in the 'statforbiology' package, which is the accompanying package for this blog. If you do not have this package, please refer to [this link](https://www.statforbiology.com/rpackages/) to download it.
...
Happy coding! And... don’t forget to check out my new book below!
