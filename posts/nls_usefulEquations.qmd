---
title: Some useful equations for biological processes
...
- Curves with maxima/minima
  1.  [Peaked sigmoidal function](#brain-function)
  2.  [Bragg function](#bragg-function)
  3.  [Lorentz function](#lorentz-function)
  4.  [Beta function](#beta-function)
...
Throughout this post, $X$ denotes the predictor, $Y$ denotes the response, and the letters of the Roman alphabet ($a$, $b$, $c$, ...) denote the model parameters, which determine the shape of the function.
...
Because of their simplicity, straight lines are mainly used to approximate biological responses over a limited range of the predictor. Should you need it, an example of fitting a straight line to research data is given elsewhere [at this link](nls_modelFitting.qmd#straight-line-function).
...
In practice, biologists often use only one branch of the parabola to approximate biological responses showing either a concave or a convex trend over a limited range of the predictor. Check [this link](nls_modelFitting.qmd#quadratic-polynomial-function) for an example of fitting.
...
The power function (named as the Freundlich equation) is widely used in agricultural chemistry, for example to model the sorption of xenobiotics in soil. It is also used to describe the relationship between the number of plant species and the sampled area (the species–area relationship). Check the post [at this link](nls_modelFitting.qmd#power-function) for an example of fitting.
...
In biology, logarithmic functions are used, for example, to describe species–area relationships in ecology, enzyme kinetics in biochemistry, and sensory perception in neurobiology. Check the post [at this link](nls_modelFitting.qmd#logarithmic-function) for an example of fitting.
...
The Michaelis-Menten function is widely used in pesticide chemistry, enzyme kinetics (Eq. 14), biological assay models (Eq. 15), and weed competition studies, where it is used to describe crop yield loss as a function of weed density (Eq. 16). An example of fitting is shown in another post, [at this link](nls_usefulEquations.qmd#rectangular-hyperbola).
...
# Sigmoidal functions {#sigmoidal-function}
...
#| message: false
#| warning: false
#| fig-cap: "The different shapes of sigmoidal curves based on x: logistic (red lines), Gompertz (blue lines) and modified Gompertz (green lines)."
#| label: fig-3
...
# Sigmoidal increase
par(mfrow = c(1,2))
c <- 2; d <- 18; b <- 0.03; e <- 115
plot(0, type="n", xlim=c(0,300), ylim=c(0,20),
     main = "Ascending sigmoids", xlab = "X", ylab = "Y")
curve(L4.fun(x, b, c, d, e) , add = T, col = "red")
curve(G4.fun(x, b, c, d, e) , add = T, col = "blue")
curve(E4.fun(x, b, c, d, e) , add = T, col = "green")

# Sigmoidal decrease
c <- 2; d <- 18; b <- -0.03; e <- 115
plot(0, type="n", xlim=c(0,300), ylim=c(0,20),
     main = "Descending sigmoids", xlab = "X", ylab = "Y")
curve(L4.fun(x, b, c, d, e) , add = T, col = "red")
curve(G4.fun(x, b, c, d, e) , add = T, col = "blue")
curve(E4.fun(x, b, c, d, e) , add = T, col = "green")
```

Each of these three families (logistic, Gompertz and modified Gompertz) can be transformed by replacing $X$ with $\log(X)$, giving raise to three corresponding families:
...
Thanks to their versatility, sigmoidal functions have been used for an uncountable number of biological applications. Just to mention a few examples in agriculture, I would like to cite plant growth, dose-response curves in biological assays and the time-course of seed germination. For some examples, take a look at my post [at this link](nls_modelFitting.qmd#sigmoidal-function).
...
$$Y = c + \frac{d - c}{\left[1 + exp(- b (X - e))\right]^{1/f}}$$

In this model, the degree of asymmetry is controlled by the parameter $f$. When $f = 1$, the function reduces to the logistic model, whereas, as $f$ decreases below 1, the curve progressively resembles a Gompertz function. Figure 4 illustrates the different shapes obtained for different values of $f$.
...
#| message: false
#| warning: false
#| fig-cap: "The different shapes of the Richard's function, based on the $f$ parameter: $f = 1$ (black line), $f = 0.5$ (blue line) and $f = 1.5$ (red line)"
#| label: fig-4

beta <- 0.1; gamma <- 50
curve(1/((1 + exp(-beta*(x - gamma)))^1), from=0, to=100,
      ylim = c(0, 1))
curve(1/((1 + exp(-beta*(x - gamma)))^(1/0.5)), add = T, col = "red")
curve(1/((1 + exp(-beta*(x - gamma)))^(1/1.5)), add = T, col = "blue")
```



# Curves with maxima/minima {#maxima-fun}
...
## Peaked sigmoidal function {#brain-function}
...
The parameters have the same interpretation as in the log-logistic curves, but $e$ does not represent the response half-way between the lower and higher asymptote and $f > 0$  represents the size of the hormetic effect, which increases as $f$ increases, while $f = 0$ corresponds to the situation of no hormesis (and the function reduces to a log-logistic.
...
$$Y = \frac{d} { 1 + b (X - e)^2 } \quad \quad \quad (25)$$
